...
interface Props {
  ...
  updateMsg?: any
  historyMsg?: Array<any> | any
  moreFunction?: Array<any>
  iconSize?: string
...
const props = withDefaults(defineProps<Props>(), {
  ...
  messageData: [{
    ...
    name: '马王',
    message: 'hello',
    avator: 'https://img-cdn-tc.dcloud.net.cn/uploads/avatar/000/15/95/31_avatar_max.jpg',
  }] as any[],
  updateMsg: {},
  historyMsg: {},
  moreFunction: [{
...
watch(() => props.updateMsg, (val) => {
  if (Object.keys(val) === 0)
    return

  selfMsgData.value.push(fixSelfData(val))
  setScroll()
}, { deep: true })

watch(() => props.historyMsg, (val, oldVal) => {
  if (val === oldVal)
    return
  ...
  if (valueType === '[object Object]') {
    ...
    let num = 0
    for (num; num < len; num++)
      selfMsgData.value.unshift(val[num])
  }
...
*/
onMounted(() => {
  selfMsgData.value = fixSelfData(JSON.parse(JSON.stringify(props.messageData)))
})
...
function setScroll() {
  const len = selfMsgData.value.length - 1
  const id = selfMsgData.value[len][props.IDType]
  // 我这里的id的设置规则是自己想的
...
function fixSelfData(arr) {
  const keyArr = Object.keys(props.dataKeys)
  const valueArr = Object.values(props.dataKeys)
...
<template>
  <view class="content">
    <scroll-view :refresher-enabled="isUseRefresh" :refresher-threshold="100" refresher-default-style="white" :refresher-triggered="isFresh" refresher-background="#F2F2F2" :enable-back-to-top="true" :show-scrollbar="false" :scroll-y="true" class="cola-message" :scroll-into-view="chatViewId" @refresherrestore="onRestore" @refresherrefresh="scrollRefresh" @tap="changeIs">
      ...
      >
        <image class="cola-message__msg__line__photo" :src="msgItem[dataKeys['avator']]" mode="" />
        <view
        ...
        <image v-else :src="msgItem[dataKeys['img']]" class="self-img" mode="" @tap="imgTap(msgItem[dataKeys['img']])" />
        <view class="isRead">
    ...
    <view :style="{'bottom':(keybordMove?'400':'0')+'rpx'}" class="bottom-keyboard">
      <view class="cola-keyboard">
        ...
        <view class="cola-keyboard__input">
          <input v-model="text" :focus="inputFocus" :disabled="inputDisabled" @tap="clickInput" @confirm="inputConfirm">
        </view>
        <!-- <image class="cola-keyboard__expression" src="" mode=""></image> -->
        <!-- <uni-icon size="40" class="cola-keyboard__expression" type="star" /> -->
        <a class="i-carbon-star text-2xl" />
